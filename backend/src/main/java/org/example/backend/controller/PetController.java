...
/**
 * REST controller for Pet resources (CRUD + actions).
 * Routes under /api/pets; CORS allowed from the Vite dev server.
 */
@RestController
...
public class PetController {
    ...
    /**
     * GET /api/pets/{id}
     * Returns a single pet by id, or 404 if not found.
     */
    @GetMapping("/{id}")
     ...
     * Returns all pets belonging to a user.
     */
    ...
    /**
     * POST /api/pets
     * Creates a pet from a DTO; requires a valid userId inside the DTO.
     * Returns 201 Created with the saved pet DTO.
     */
    @PostMapping
    ...
    public PetDto createPet(@RequestBody PetDto petDto) {
        ...
        if (owner == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "User not found: " + petDto.getUserId());
        }
    ...
    /**
     * POST /api/pets/adopt
     * Convenience endpoint to adopt a new pet by name/type for a given user.
     ...
     * Feeds the pet and returns the updated DTO.
     */
    @PostMapping("/{id}/feed")
     ...
     * Plays with the pet and returns the updated DTO.
     */
    @PostMapping("/{id}/play")
     ...
     * Rests the pet and returns the updated DTO.
     */
    @PostMapping("/{id}/rest")
    ...
    /**
     * DELETE /api/pets/{id}
     * Deletes a pet. Returns 204 No Content.
     */
    @DeleteMapping("/{id}")
